"""Risk Mapper — produce a flood risk report from a trained FloodLSTM."""
...
def _make_tabular_stub(cfg: dict[str, Any], device: torch.device) -> torch.Tensor:
    """Return a random normalised input tensor for demo inference.

    Shape: (1, window_size, 20) — mimics one sliding-window sample.

    If `inference.default_features` points to a JSON file containing a dict
    of {feature_name: value} pairs, those values are used instead.
    """
    ...
    if feature_path_str:
        ...
        if feat_path.exists():
            ...
            # Tile the same feature vector over the window
            seq = np.tile(row, (window_size, 1))
            return torch.from_numpy(seq).unsqueeze(0).to(device)  # (1, W, F)

    # No features file → use random stub
